...
class TestSkillValidation:
    ...
    def test_warns_on_unknown_skill(self, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            mgr.validate_skills({"backstab", "circle"})
        assert any("nonexistent skill" in record.message for record in caplog.records)

    def test_passes_for_known_skills(self, caplog):
        ...
        mgr = CombatSkillManager(agent)
        with caplog.at_level(logging.WARNING):
            mgr.validate_skills({"backstab", "kick"})
        warning_records = [r for r in caplog.records if r.levelno >= logging.WARNING]
        assert len(warning_records) == 0

    def test_empty_learned_skills_warns_all(self, caplog):
        """All configured skills generate warnings when learned set is empty."""
        import logging
        ...
        mgr = CombatSkillManager(agent)
        with caplog.at_level(logging.WARNING):
            mgr.validate_skills(set())
        warning_messages = [r.message for r in caplog.records if r.levelno >= logging.WARNING]
        assert any("backstab" in m for m in warning_messages)
        assert any("kick" in m for m in warning_messages)
